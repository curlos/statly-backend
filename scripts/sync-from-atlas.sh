...
# Sync Atlas M0 → Local MongoDB
# Exports Atlas data and imports to local (overwrites local data)

set -e  # Exit on error

echo "🔄 Syncing Atlas M0 → Local MongoDB"
echo "===================================="
echo ""
...
if mongodump --uri="$ATLAS_URI" --out="$BACKUP_DIR" > /dev/null 2>&1; then
    ...
    echo -e "${RED}✗${NC} Failed to export from Atlas M0"
...
echo ""
echo "Atlas M0 → Local MongoDB sync successful"
